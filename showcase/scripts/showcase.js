...
function printDecision(decision) {
    ...
    const statusColor = decision.status === "Approved" ? chalk.green :
    ...
    if (decision.findings && decision.findings.length > 0) {
        ...
        decision.findings.forEach((finding, index) => {
    ...
    if (decision.trace && decision.trace.length > 0) {
        ...
        decision.trace.forEach((step, index) => {
...
function printSummary(results) {
    ...
    results.forEach((result, index) => {
        ...
        const actualStatus = result.decision?.status === "Approved" ? chalk.green(result.decision.status) :
